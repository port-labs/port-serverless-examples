...
def report_to_port(entity_props: dict):
    ...
    entity = {
        ...
        'properties': {
            'version': entity_props['version'],
            'type': entity_props['type']
        },
        "relations": {
            "RegionHasManyClusters": entity_props['region']
        }
    }
...
def update_cluster_in_port(entity_props: dict, entity_identifier: str):
    '''
    Reports to Port on a new entity based on provided ``entity_props``
    '''
    logger.info('Fetching token')
    ...
    logger.info(json.dumps(entity))
    response = requests.patch(f'{API_URL}/blueprints/{blueprint}/entities/{entity_identifier}', json=entity, headers=headers)
    logger.info(response.status_code)
    logger.info(json.dumps(response.json()))
    ...
    for messages in event['records'].values():
        for encoded_message in messages:
            try:
                ...
                elif action_type == UPDATE_TRIGGER:
                    status_code = update_cluster_in_port(message['payload']['properties'], message['context']['entity'])
